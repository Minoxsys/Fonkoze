    ...
    public class ReceiveSmsMethod
    {
        ...
        public void ReceiveSms_SavesAnErrorAlert_WhenMessageIsIncorrect()
        {
            ...
            objectMother.saveAlertCmd.AssertWasCalled(
                cm => cm.Execute(Arg<Alert>.Matches(a => a.AlertType == AlertType.Error && a.Client == objectMother.outpost.Client &&
        ...
        [Test]
        public void ReveiceSms_ReturnsNothing_WhenSenderIsUnknown()
        {
            //arange
            objectMother.smsGatewayService.Stub(s => s.AssignOutpostToRawSmsReceivedBySenderNumber(Arg<RawSmsReceived>.Is.Anything))
                        .Return(new RawSmsReceived {OutpostId = Guid.Empty});

            //act
            var result = objectMother.controller.ReceiveSms(ObjectMother.MOBILE_NUMBER, ObjectMother.SMS_CONTENT, ObjectMother.IN_NUMBER, ObjectMother.EMAIL,
                                                            ObjectMother.CREDITS);
