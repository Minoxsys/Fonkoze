...
namespace Tests.Unit.Security
{
	...
	public class RequiresAttributeTests
	{
		...
		private static AuthorizationContext FakeAuthorizationContext_UserSignedIn_And_DOCUMENTUPLOADER_FunctionRightAssigned()
		{
			return FakeAuthorizationContext(
				(httpContext, functionRightsService) =>
				{
					...
					A.CallTo(() => functionRightsService.HasPermissionAssigned(A<string>.That.IsEqualTo(DOCUMENT_UPLOADER),principal.Identity.Name)).Returns(true);
				}
				);
		}
		...
		{
			Action<HttpContextBase, IPermissionsService> assign_unsigned_user = (httpContext, functionRightsService) =>
				{
		...
		private static AuthorizationContext FakeAuthorizationContext_UserSignedIn_DOCUMENTUPLOADER_NotAssigned()
		{
			...
			Action<HttpContextBase, IPermissionsService> assign_a_signed_in_user = (httpContext, functionRightsService) =>
				{
		...
		private static AuthorizationContext FakeAuthorizationContext(Action<HttpContextBase, IPermissionsService> establishWorkingParameters)
		{
			...
			var functionRightsService = A.Fake<IPermissionsService>();
			...
			A.CallTo(() => functionRightsService.HasPermissionAssigned(A<string>.That.IsEqualTo(DOCUMENT_UPLOADER),httpContext.User.Identity.Name)).Returns(false);

			A.CallTo(() => container.Resolve(typeof(IPermissionsService), null)).WithAnyArguments().Returns(functionRightsService);
